...
function Nav({toggle,userFound}) {
  ...
  return (
    <div className=' h-16 bg-slate-100 flex justify-between items-center font-bold px-4'>
        <div className='text-3xl hover:cursor-pointer hover:text-green-500  ' onClick={()=>{navigate("/home")}}>
            Sumit
        </div>
        ...
        <div className={showdropdown ?'absolute md:hidden bg-slate-300  rounded top-14 left-0 w-full z-10':'hidden md:flex justify-between w-1/2'}>
            <div className={showdropdown ?'px-4 flex items-center h-12 text-xl hover:bg-blue-500 hover:text-white  rounded':'hover:cursor-pointer  hover:text-blue-500 transform hover:scale-110 transition-transform duration-200'}   onClick={()=>{navigate("/home");setShowdropdown(false)}}> <i class="fa-solid fa-house"></i> Home</div>
           { userFound.admin&&<>
            ...
            <div className={showdropdown ?'px-4 flex items-center h-12 text-xl text-red-600 hover:bg-blue-500 hover:text-white rounded':'hover:cursor-pointer  hover:text-blue-500 transform hover:scale-110 transition-transform duration-200'} onClick={()=>{navigate('/');toggle();localStorage.clear();}}> Logout</div>
        </div>
    </div>
  )
}
